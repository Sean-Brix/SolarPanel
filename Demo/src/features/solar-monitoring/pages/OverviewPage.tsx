...
import { getAnnLatest } from '@/features/solar-monitoring/data/ann-source'
import {
  conventionalReadings,
  estimateHumidityForTimestamp,
  estimateIrradianceForTimestamp,
  estimateTemperatureForTimestamp,
  fixedReadings,
  getAllReadingsSince,
  queryReadings,
...
import type { HistoryRow, PaginationInfo, PanelKey } from '@/shared/types/solar'

type BackendReading = RawFixedReading | RawTrackerReading
...
function panelReadingsSource(panel: ComparisonPanelKey): BackendReading[] {
  return panel === 'fixed' ? fixedReadings : conventionalReadings
}

function isTrackerBackendReading(reading: BackendReading): reading is RawTrackerReading {
  return 'axisX' in reading
}

function normalizeReading(panel: ComparisonPanelKey, item: BackendReading): NormalizedReading {
  ...
  return {
    ...
    power: item.power,
    irradiance: estimateIrradianceForTimestamp(timestamp),
    temperature: estimateTemperatureForTimestamp(timestamp),
    humidity: estimateHumidityForTimestamp(timestamp),
    azimuthAngle: isTrackerBackendReading(item) ? item.axisY : undefined,
    elevationAngle: isTrackerBackendReading(item) ? item.axisX : undefined,
  }
}

function fetchAllOverviewPanelReadings(panel: ComparisonPanelKey, since?: Date): NormalizedReading[] {
  const readings = getAllReadingsSince(panelReadingsSource(panel), since)
  return readings.map((item) => normalizeReading(panel, item))
}
...
export function OverviewPage() {
  ...
  useEffect(() => {
    const fetchPanelReadings = (panel: ComparisonPanelKey, page: number, pageSize: number, since?: Date) => {
      const source = since
        ? getAllReadingsSince(panelReadingsSource(panel), since)
        : panelReadingsSource(panel)
      const result = queryReadings(source, { page, pageSize })

      const readings = result.items
        .map((item) => normalizeReading(panel, item))
    ...
    const fixedResult = fetchPanelReadings('fixed', fixedHistoryPage, fixedHistoryPageSize, since)
    const conventionalResult = fetchPanelReadings(
      'conventional',
      conventionalHistoryPage,
      conventionalHistoryPageSize,
      since,
    )
    const latestAnn = getAnnLatest()
