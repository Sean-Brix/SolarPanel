...
import { conventionalReadings, getAllReadingsSince } from '@/features/solar-monitoring/data/panel-readings-source'
import { buildDaySheets, exportWorkbookByDay } from '@/shared/lib/excelExport'
import type { TimeRange } from '@/shared/types/solar'
import CONVE_EXCEL_URL from '@/assets/CONVE.xlsx?url'
...
function fetchAllConventionalReadings(range: TimeRange) {
  const since = rangeToSinceDate(range)
  return getAllReadingsSince(conventionalReadings, since)
}
...
export function ConventionalPanelPage() {
  ...
  return (
    <div className="space-y-4 pb-10">
      <FindingsModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} excelUrl={CONVE_EXCEL_URL} title="Conventional Panel" />
      <PageHeader
