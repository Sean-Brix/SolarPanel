 ...
 * Data source: MQTT topic helios/readings/fixed
...
/**
 * GET /api/fixed/history?page=1&pageSize=50&since=ISO_DATE
 * Data source: MQTT topic helios/readings/fixed
 * Returns paginated Fixed panel readings, newest first.
 * Query params:
...
router.get('/history', async (req, res) => {
  ...
  const totalCount = await prisma.fixedReading.count({ where })
